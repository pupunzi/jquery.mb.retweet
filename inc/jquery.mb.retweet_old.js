...
(function($){
  ...
  $.mbRetweet={
    ...
    mbBitlyShorten:function(){
      var el=$(this).get(0);
      var url="http://api.bit.ly/shorten?version="+$.mbRetweet.BitlyAPIversion+"&longUrl="+encodeURIComponent(el.url)+"&login="+$.mbRetweet.user+"&apiKey="+$.mbRetweet.key;
      $.ajax({
        url:url,
        dataType: "jsonp",
    ...
    buildMbRetweet:function(options){
      return this.each(function(){
        ...
        this.url= $(this).hasClass("self")?self.location.href:$(this).attr("href");
        this.desc= $(this).text();
        ...
        if ($.mbRetweet.user){
          ...
          var checkUrl = setInterval(function(){
            if ( typeof el.shortUrl !== "undefined" ) {
              ...
              $(el).find(".mbretweetLabel").css("background-color",el.options.color).hover(
                function(){
                  $(this).css("opacity",.7);
                  clearTimeout(el.removeExtra);
                },
                function(){
                  $(this).css("opacity","");
                  el.removeExtra=setTimeout(function(){$(el).removeMbRetweetExtra();},1000);
                });
              ...
              $(el).find(".mbretweetLabel").one("click",function(){
                ...
                var twitterUrl="http://twitter.com/home?status=" +	encodeURIComponent(el.options.prefix + " " + el.desc + ": " +el.shortUrl);
          ...
          $(el).find(".mbretweetLabel").css("background-color",el.options.color).hover(
            ...
            function(){
              el.removeExtra=setTimeout(function(){$(el).removeMbRetweetExtra();},500);
            });
          $(el).find(".mbretweetLabel").one("click",function(){
            if($("#mbretweetDesc").length>0) {
              el.desc= $("#mbretweetDesc").val();
            }
            var twitterUrl="http://twitter.com/home?status=" +	encodeURIComponent(el.options.prefix + " " + el.desc + ": " +el.url);
            window.open(twitterUrl,"twitter");
    ...
    buildMbRetweetExtra:function(){
      var el=$(this).get(0);
      var tweetUrl=$.mbRetweet.user?el.shortUrl:el.url;
      clearTimeout(el.removeExtra);
      ...
      $(this).after(extracontent);
      extracontent.fadeIn();
      $("#mbretweetDesc").css({color:el.options.color}).focus();/*.select()*/
      $(".mbretweetExtra").bind("mouseenter",
        function(){
          clearTimeout(el.removeExtra);
        })
        .bind("mouseleave",
        function(){
          el.removeExtra=setTimeout(function(){
            $(el).removeMbRetweetExtra();
          },500);
        });
